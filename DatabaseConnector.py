...
import mysql.connector, pymysql, csv
    ...
    try:
        ...
        with open(path, mode = 'r') as csv_file:
            ...
            for row in csv_data:
                ...
                insert = sql_single_insert % (row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7])
                print(insert)
...
def average( table, column ):
    try:
        ...
        # print will be removed after final script
        # only used for testing
        print("SELECT avg(%s) from %s" % (column, table))
